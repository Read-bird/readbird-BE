...
export const refresh = async (req: Request, res: Response) => {
    if (req.headers.authorization && req.headers.refresh) {
        ...
        //refreshToken 반환
        const refreshToken = await redis.redisCli.get(
        ...
        if (authResult.ok === false && authResult.message === "jwt expired") {
            ...
            if ((await refreshResult) == false) {
                ...
                return res
                    .header("accesstoken", "Bearer " + newAccessToken)
                    .status(200);
            }
        } else {
            ...
            return res
                .status(400)
                .send("Bad Request: accessToken이 만료 되지 않음");
        }
    } else {
        ...
        return res
            .status(400)
            .send(
                "Bad Request: header에 accessToken 또는 refreshToken이 존재 하지 않음",
            );
    }
};
